...
class Updater {
  ...
  _httpsGet(url) {
    return new Promise((resolve, reject) => {
      const options = {
        headers: { "User-Agent": "G-Attendance-Updater" },
      };
  ...
  _getAppPath() {
    // process.execPath is e.g. /path/to/G-Attendance.app/Contents/MacOS/G-Attendance
  ...
  async downloadUpdate() {
    ...
    try {
      ...
      if (!zipAsset) {
        this._send("update:error", {
          message: "No ZIP asset found in release",
        });
        return;
      }

      const tmpDir = path.join(os.tmpdir(), "g-attendance-update");
      ...
      if (!this.downloadedAppPath) {
        this._send("update:error", {
          message: "No .app found in update ZIP",
        });
        return;
      }
